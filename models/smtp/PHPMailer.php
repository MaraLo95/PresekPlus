...
class PHPMailer
{
     ...
     * @return bool
     */
    private function mailPassthru($to, $subjects, $body, $header, $params)
    {
        //Check overloading of mail function to avoid double-encoding
        if (ini_get('mbstring.func_overload') & 1) {
            $subjects = $this->secureHeader($subjects);
        } else {
            $subjects = $this->encodeHeader($this->secureHeader($subjects));
        }
        //Calling mail() with null params breaks
        if (!$this->UseSendmailOptions or null === $params) {
            $result = @mail($to, $subjects, $body, $header);
        } else {
            $result = @mail($to, $subjects, $body, $header, $params);
        }
